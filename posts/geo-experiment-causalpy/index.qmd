...
The regions share the same broad seasonal pattern. That co-movement is what makes a synthetic control plausible, but it is not enough on its own. The pre-treatment fit still needs to be checked.

### South East January transactions, year on year

Comparing the weeks around 1 January shows that the spike is recurring. January 2026 still rises sharply even though advertising is off. The switch-off lowers the peak, but does not remove it.
...
The donor weights are non-negative and sum to one. The Bayesian model also carries uncertainty from the pre-period into the post-period counterfactual.
...
The model converged cleanly, with no divergences, a maximum R-hat of 1.003 and a minimum bulk effective sample size of 1,938. More importantly for design validity, the pre-treatment correlation between the observed and synthetic South East was **0.968**.
...
The package output shows the observed and counterfactual series, the weekly impact, and the cumulative impact. The gap opens after advertising is switched off and accumulates across the ten-week test.
...
The full-history plot is useful for validation. A closer view makes the commercial result easier to see: actual transactions remain high, but they sit below the counterfactual during the switch-off.
